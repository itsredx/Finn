...
const TransactionRow: React.FC<{ transaction: Transaction }> = ({ transaction }) => {
  const isPayout = transaction.type === TransactionType.Payout;
    ...
    <tr className="border-b border-gray-200 last:border-b-0 hover:bg-gray-50">
      <td className="px-6 py-5">
        <div className="flex items-center gap-4">
          <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isPayout ? 'bg-gray-200' : 'bg-gray-200'}`}>
            {isPayout ? <ArrowUpIcon className="text-[#214D76]" /> : <ArrowDownIcon className="text-[#214D76]" />}
          </div>
          <div>
