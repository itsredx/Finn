...
const EmptyState: React.FC<{ onOpenModal: () => void }> = ({ onOpenModal }) => (
    <div className="bg-white rounded-lg border-2 border-dashed border-gray-300 p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
        <BankAccountIcon />
        <h2 className="mt-6 text-2xl font-bold text-gray-800">No virtual accounts yet.</h2>
        <p className="mt-2 text-base text-gray-500">Create your first account to get started.</p>
        <button
            onClick={onOpenModal}
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1000px]">
          <thead className="bg-[#EEF0F0]">
            <tr>
              <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider">Account Name</th>
              <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider">Purpose</th>
              <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider">Currency</th>
              <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500 uppercase tracking-wider">Bank</th>
            ...
            {accounts.map((account) => (
              <tr key={account.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                    <p className="font-semibold text-base text-black">{account.accountName}</p>
                </td>
                 <td className="px-6 py-4 whitespace-nowrap">
                    <p className="text-base text-gray-600">{account.purpose === 'inbound_invoices' ? 'Inbound Invoices' : 'Settlements'}</p>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
      ...
      <main className="p-8 lg:p-16">
        <div className="max-w-7xl mx-auto space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-4xl font-bold text-[#214D76]">Virtual Accounts</h1>
                    <p className="text-lg text-gray-500 mt-2">Manage your virtual bank account incoming transfers.</p>
                </div>
                {accounts.length > 0 && (
                     <button 
